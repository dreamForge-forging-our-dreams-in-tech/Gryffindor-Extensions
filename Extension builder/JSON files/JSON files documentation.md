...
This registery is only available if the extension has been installed else itll crash, so be sure to wrap it in a extra check because extensions wont load if the registery is not available.
